...
構成: 9 fold-unit × 3 seed = 27 ラン · 実行環境: A100-SXM4-40GB / 1 GPU per job · 不確実性チェック: 27 / 27 合格 · commit: c2494a8

---

- **AUC の低下幅**: −0.046 — 分布外 0.860 ± 0.028 ／ 分布内 0.906 ± 0.014。 AUPRC は見かけ上ほぼ同じだが、有病率で正規化した lift は 4.09 → 2.62 と 36% 落ちる。 ✓ 主張は成立
...
**ただし AUPRC を「差がほぼ無い」と読んではいけない。** AUPRC の下限は有病率であり、分布外のほうが有病率が高い（27.8% 対 18.0%）。 有病率で正規化した lift で測ると **4.09 → 2.62、36% の低下**である。 生の AUPRC 0.738 → 0.727 がほぼ横ばいに見えるのは、 底上げされたベースラインに助けられているだけで、頑健性の証拠にはならない。
...
vendor（Philips）の S の絶対値が 20〜37 と他ユニット（40〜60）より低いことを、 当初は「最も分布の遠いシフトに自発的に自信を下げている」と読んだが、 **これは誤りである。S の絶対値はラン間で比較できない**—— 訓練セットの大きさも構成も違うので、水準そのものが揃っていない。 同一モデル内で inner-val と test を比べると vendor は −10%、LOSO-B は −9%、field は +5% で、 **vendor が特別に敏感なわけではない**（§14.2 の弱く不安定な反応と整合する）。 この段落の主張は「誤答時に S が下がる」——同一モデル・同一集団内の比較——に限られる。
